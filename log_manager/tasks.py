...
from resources.models import RobotUserAgent, MMDB
...
def task_discover_logs(self, collection_acron2, days_to_go_back=None, from_date=None, user_id=None, username=None):
    ...
    if len(col_configs_dirs) == 0:
        # tracker_models.Event.create(
        #     user=self.request.user,
        #     message_type=tracker_choices.ERROR,
        #     message=_('CollectionLogDirectory is missing.'),
        #     detail={
        #         'Action': _('Add a CollectionLogDirectory for the collection.'),
    ...
    if len(supported_logfile_extensions) == 0:
        # tracker_models.Event.create(
        #     user=self.request.user,
        #     message_type=tracker_models.choices.ERROR,
        #     message=_(f'SupportedLogfile is missing.'),
        #     detail={
        #         'Action': _('Add a SupportedLogFile to indicate the supported log file format.'),
        #         'Result': _('The collection has been ignored.'),
        #         'Collection': collection_acron2,
        #     }
        # )
    ...
    if days_to_go_back:
        ...
        try:
            ...
            #     user=self.request.user,
            #     message_type=tracker_models.choices.ERROR,
            #     message=_('Invalid date format usage.'),
            #     detail={
...
def task_validate_log(self, log_file_hash, user_id=None, username=None):
    ...
    if val_results.get('is_valid', {}).get('all', False):
        ...
        # TODO: Add a message to the tracker
...
def task_check_missing_logs_for_date(self, collection_acron2, date, user_id=None, username=None):
    ...
    try:
        ...
        #     user=self.request.user,
        #     message_type=tracker_models.choices.ERROR,
        #     message=_('CollectionLogFilesPerDay is missing.'),
        #     detail={
        #         'Action': _('Add a CollectionLogFilesPerDay for the collection.'),
        #         'Result': _('The collection has been ignored.'),
        #         'Collection': collection_acron2,
        #         'Date': date,
        #     })
        ...
        #     user=self.request.user,
        #     message_type=tracker_models.choices.ERROR,
        #     message=_('There are two or more configurations for the same date.'),
        #     detail={
        #         'Action': _('Keep only one configuration enabled for the number of expected files on a single date.'),
        #         'Result': _('The collection has been ignored.'),
        #         'Collection': collection_acron2,
        #         'Date': date,
        #     })
...
def task_send_message(self, subject, message, collection_acron2, user_id=None, username=None):
    ...
    if len(collection_emails) == 0:
        ...
        #     user=self.request.user,
        #     message_type=tracker_models.choices.ERROR,
        #     message=_('CollectionConfig is missing.'),
        #     detail={
        #         'Action': _('Add an Application Configuration for the EMAIL attribute.'),
        #         'Result': _('The message has not been sent.'),
        #         'Collection': collection_acron2,
        #     })
...
def task_parse_log(self, log_file_hash, user_id=None, username=None):
    ...
    if not robots_list:
        logging.error('ERROR. There are no robots available in the database.')
        log_file.status = choices.LOG_FILE_STATUS_QUEUED
        log_file.save()
        return

    mmdb = MMDB.objects.last()
    if not mmdb:
        logging.error('ERROR. There are no MMDB files available in the database.')
        log_file.status = choices.LOG_FILE_STATUS_QUEUED
        log_file.save()
        return
    ...
    try:
        for row in utils.parse_file(
            path_file=log_file.path,
            mmdb_data=mmdb.data,
            robots_list=robots_list,
        ):
            '''
            Each row is a tuple with the following structure:
                hit.local_datetime
                hit.client_name
                hit.client_version
                hit.ip
            ...
            # latlong is a string with the following format: 'latitude\tlongitude'
            lat, long = latlong.split('\t')

            # FIXME: Should we improve the model with a country_code label?
            # FIXME: It is a good idea to convert the action_name to its respective components (increasing the LogProcessedRow model)?
